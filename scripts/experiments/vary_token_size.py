...
for num_buffers in [1,2,4,8]:
    ...
    for token_size in [4096*4**x for x in range(6)]:
        ...
        subprocess.call("../config/mod " + "-minni__tbb__buffers " + str(num_buffers) + " -minni__tbb__token_size " + str(token_size) + " -minni__tbb__max_keys_per_token " + str(num_tokens), shell=True)
