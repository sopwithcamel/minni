...
EHFullEvict::EHFullEvict(uint64_t cap, uint64_t pid)
{
	partid = pid;
	capacity = cap;
	dumpNumber = 0;	
...
EHFullEvict::~EHFullEvict()
{
}

void EHFullEvict::setSerializeFormat(int sformat)
{
 ...
 */
bool EHFullEvict::insert(string key, PartialAgg* pao)
{
...
bool EHFullEvict::add(const string &key, const string &value)
{
//	cout << "Key: " << key << endl;
...
Hash::iterator EHFullEvict::find(const string &key)
{
	return hashtable.find(key);
}

void EHFullEvict::serialize(FILE* fileOut, string key, string value)
{
...
void EHFullEvict::serialize(FILE* fileOut, char type, uint64_t keyLength, const char* key, uint64_t valueLength, const char* value)
{
 ...
 * call dumpHashtable */
bool EHFullEvict::finalize(string fname)
{
	if (evictFile == NULL)
...
bool EHFullEvict::dumpHashtable()
{
 ...
 */
string EHFullEvict::getDumpFileName(uint64_t dn)
{
	stringstream ss;
...
bool EHFullEvict::clear()
{
	hashtable.clear();
}

int EHFullEvict::deSerialize(FILE* fileIn, char* type, uint64_t* keyLength, char** key, uint64_t* valueLength, char** value)
{
	size_t result;
