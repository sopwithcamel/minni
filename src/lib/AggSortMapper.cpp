...
void Mapper::ExternalSort(string unsorted, string sorted)
{
...
void Mapper::Map(MapInput* input) {
	...
	for (int i=0; i < num_partition; i++)
		aggregs[i]->setSerializeFormat(REGULAR_SERIALIZE);

	cout << "Emitting to aggregators" << endl;
	ifstream fstr(sortedagg.c_str());
