...
ExtendableMap::ExtendableMap(uint64_t cap, uint64_t pid)
{
	partid = pid;
	capacity = cap;
	dumpNumber = 0;	
...
ExtendableMap::ExtendableMap(uint64_t capacity, uint64_t partid, const string &path)
{
	partid = partid;
	capacity = capacity;
	dumpNumber = 0;
 ...
 */
bool ExtendableMap::insert(string key, PartialAgg* pao)
{
...
bool ExtendableMap::add(const string &key, const string &value)
{
...
map<string,PartialAgg*>::iterator ExtendableMap::find(const string &key)
{
	return hashtable.find(key);
}

void ExtendableMap::serialize(FILE* fileOut, char type, uint64_t keyLength, const char* key, uint64_t valueLength, const char* value)
{
 ...
 * call dumpHashtable */
bool ExtendableMap::finalize(string fname)
{
...
bool ExtendableMap::dumpHashtable(string fname)
{
 ...
 */
string ExtendableMap::getDumpFileName(uint64_t dn)
{
	stringstream ss;
...
bool ExtendableMap::clear()
{
	hashtable.clear();
}

int ExtendableMap::deSerialize(FILE* fileIn, char* type, uint64_t* keyLength, char** key, uint64_t* valueLength, char** value)
{
	size_t result;
