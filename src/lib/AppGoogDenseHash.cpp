...
AppGoogDenseHash::AppGoogDenseHash(uint64_t cap, uint64_t pid)
{
	hashtable.set_empty_key("q-1q1--1q1");
	partid = pid;
	capacity = cap;
	dumpNumber = 0;	
}
...
AppGoogDenseHash::AppGoogDenseHash(uint64_t capacity, uint64_t partid, const string &path)
{
	hashtable.set_empty_key("q-1q1--1q1");
	partid = partid;
	capacity = capacity;
...
AppGoogDenseHash::~AppGoogDenseHash()
{
}
 ...
 */
bool AppGoogDenseHash::insert(string key, PartialAgg* pao)
{
...
bool AppGoogDenseHash::add(const string &key, const string &value)
{
...
google::dense_hash_map<string, PartialAgg*>::iterator AppGoogDenseHash::find(const string &key)
{
	return hashtable.find(key);
}

void AppGoogDenseHash::serialize(FILE* fileOut, char type, uint64_t keyLength, const char* key, uint64_t valueLength, const char* value)
{
 ...
 * call dumpHashtable */
bool AppGoogDenseHash::finalize(string fname)
{
...
bool AppGoogDenseHash::dumpHashtable(string fname)
{
/*
	FILE* fptr = fopen(fname.c_str(), "wb");
	google::dense_hash_map<string, PartialAgg*>::iterator aggiter;
 ...
 */
string AppGoogDenseHash::getDumpFileName(uint64_t dn)
{
	stringstream ss;
	ss << "/dev/null";
//	ss << dn;
...
bool AppGoogDenseHash::clear()
{
	hashtable.clear();
}

int AppGoogDenseHash::deSerialize(FILE* fileIn, char* type, uint64_t* keyLength, char** key, uint64_t* valueLength, char** value)
{
	size_t result;
